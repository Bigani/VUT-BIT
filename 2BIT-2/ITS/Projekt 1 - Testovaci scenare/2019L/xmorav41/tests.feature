Feature: Internet shop OpenCart; User Experience
  ...
  Scenario: User's information edit
    Given User is on Account Information tab
    When User changes informations
    Then Informations are updated

  Scenario: User's enquiry
  ...
  Scenario: Subscripton change
    Given User is on Newsletter Subscripton page
    When User changes subscribe option and continues
    Then User recieves feedback about subsription update
  ...
  Scenario: Invalid checkout no.3
    ...
    When User removes items form cart
