...
define("SYM", '/^string@(\\\\[0-9]{3}|[^\\\\])*$|(^nil@nil$)|(^bool@false$|^bool@true$)|(^int@([+-]?[1-9][0-9]*|[+-]?0$))/');
define("SYMVAR",'/(L|G|T)F@(_|\-|\$|&|%|\*|!|\?|[a-zA-Z])[\S]*|(^string@(\\\\[0-9]{3}|[^\\\\])*$|(^nil@nil$)|(^bool@false$|^bool@true$)|(^int@([+-]?[1-9][0-9]*|[+-]?0$)))/');
define("LABEL", '/^(?!(int|bool|string)$)(_|\-|\$|&|%|\*|!|\?|[a-zA-Z])(_|\-|\$|&|%|\*|!|\?|[a-zA-Z0-9])*$/');
...
function Process_args($argc,$argv,$args){
  ...
  elseif(($argc == 2) && ($argv[1] == "--help")){
    echo "\n";
    echo "Skript typu filtr (parse.php v jazyce PHP 7.4) načte ze standardního vstupu zdrojový kód v IPP-code20, zkontroluje lexikální a syntaktickou správnost kódu a vypíše na standardní výstup XML reprezentaci programu.\n";
    echo "\n";
    exit(0);
  }
...
//@brief Generates xml code for instruction, inputs are already safe, flexible for every instruction type
...
function Generate_inst($xml,$program,$counter,$arg0,$arg1,$arg2,$arg3){
  ...
  if ($arg2 != NULL){
    ...
    elseif(preg_match(SYM,$arg2) == 1){//int/bool/string type inst
      ...
      $arg22 = $xml->createElement("arg2", htmlentities($arg2_piece[1]));
...
//header check//
$line = trim($line);
if (strcasecmp($line,".IPPcode20") != 0){
...
while($line = fgets($f)) {
  ...
  switch($line_piece[0]){
    ...
    case "DEFVAR":
    case "POPS":
    if (count($line_piece) != 2)
      Exit_error_lex($line,23);
    else {
      if (preg_match(VARR,$line_piece[1]) == 0 || count($line_piece) != 2 )
      {
        Exit_error_lex($line,23);
      }
      else
    ...
    if (count($line_piece) != 2)
      ...
      if ( preg_match((SYMVAR),$line_piece[1]) == 0  || count($line_piece) != 2 )
      {
        Exit_error_lex($line,23);
      }
      else
    ...
    if (count($line_piece) != 2)
      ...
      if (preg_match(LABEL,$line_piece[1])  == 0 || count($line_piece) != 2 )
      {
        Exit_error_lex($line,23);
      }
      else
    ...
    case "STRLEN":
    case "TYPE":
    if (count($line_piece) != 3)
      Exit_error_lex($line,23);
    else {

      if (preg_match(VARR,$line_piece[1]) == 0 || preg_match(SYMVAR,$line_piece[2]) == 0 || count($line_piece) != 3)
      {
        //echo print_r($line_piece);
        Exit_error_lex($line,23);
      }
      else
      {
    ...
    if (count($line_piece) != 3)
      ...
      if (preg_match(VARR,$line_piece[1])  == 0 || preg_match(TYPE,$line_piece[2]) == 0 || count($line_piece) != 3 )
      {
        Exit_error_lex($line,23);
      }
      else
    ...
    case "SETCHAR":
    case "STRI2INT":
    if (count($line_piece) != 4)
      Exit_error_lex($line,23);
    else {

      if (preg_match(VARR,$line_piece[1]) == 0 || preg_match((SYMVAR),$line_piece[2]) == 0 || preg_match((SYMVAR),$line_piece[3]) == 0 || count($line_piece) != 4)
      {
        Exit_error_lex($line,23);
      }
      else
    ...
    if (count($line_piece) != 4)
      ...
      if (preg_match(LABEL,$line_piece[1])  == 0 || preg_match((SYMVAR),$line_piece[2]) == 0 || preg_match((SYMVAR),$line_piece[3]) == 0 || count($line_piece) != 4)
      {
        Exit_error_lex($line,23);
      }
      else
