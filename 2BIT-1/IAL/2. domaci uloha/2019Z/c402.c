...
**/
    ...
    tBTNodePtr goingThrough, insertFather;
    goingThrough = *RootPtr;
    insertFather = goingThrough;
    int whichSide = 0;

    while (goingThrough != NULL){                   //Prechod stromom a hľadanie správneho listu na vloženie
        if (goingThrough->Cont < Content){          //Hodnota je väčšia, hľadáme vpravo
            insertFather = goingThrough;
            goingThrough = goingThrough->RPtr;
            whichSide = 1;
        }
        else if (goingThrough->Cont > Content){     //Hodnota je menšia, hľadáme vľavo
            insertFather = goingThrough;
            goingThrough = goingThrough->LPtr;
            whichSide = 2;
        }
        else if (goingThrough->Cont == Content){    //Uzol s hodnotou už existuje
            return;
        }
    }

    tBTNodePtr inserted = (tBTNodePtr) malloc(sizeof(struct tBTNode)); //Nový uzol
    if (inserted == NULL) return;

    inserted->Cont = Content;
    inserted->RPtr = NULL;
    inserted->LPtr = NULL;

    if (whichSide == 0) *RootPtr = inserted;            //Uzol bude koreňom
    if (whichSide == 1) insertFather->RPtr = inserted;  //Uzol bude na pravo
    if (whichSide == 2) insertFather->LPtr = inserted;  //Uzol bude na ľavo
}
...
**/
    ...
    tStackP s;
    SInitP(&s);
    tBTNodePtr passPtr = RootPtr;             //Pomocný ukazovateľ pri prechode stromom

    Leftmost_Preorder(passPtr,&s);
    while(!SEmptyP(&s)){                      //Dokým nie je zásobník prázdny prechádzaj uzlami
        passPtr = STopPopP(&s);
        Leftmost_Preorder(passPtr->RPtr,&s);  //Prechod pravými vetvami
    }
...
**/
    ...
    tStackP s;
    SInitP(&s);
    tBTNodePtr passPtr;             //Pomocný ukazovateľ pri prechode stromom

    Leftmost_Inorder(RootPtr,&s);
    while(!SEmptyP(&s)){                      //Dokým nie je zásobník prázdny prechádzaj uzlami
        passPtr = STopPopP(&s);
        BTWorkOut(passPtr);
        Leftmost_Inorder(passPtr->RPtr,&s);  //Precho pravými vetvami
    }
}
...
**/
    ...
    Leftmost_Postorder(passPtr,&s,&b);

    while(!SEmptyP(&s)){
        passPtr = STopPopP(&s);

        if (STopPopB(&b)) {                   //Ak je TRUE tak uzol vloží späť na zásobník a pridá mu bool FALSE
            SPushP(&s,passPtr);
            SPushB(&b, FALSE);
            Leftmost_Postorder(passPtr->RPtr, &s, &b);
        }
        else {
            BTWorkOut(passPtr);
        }
...
**/
    ...
    do {				//Aspoň raz prejde stromom
        if (*RootPtr == NULL){		
            *RootPtr = STopPopP(&s);	//Zoberie uzol zo zásobníka
            continue;
        }
        else if ((*RootPtr)->RPtr != NULL){
            SPushP(&s,(*RootPtr)->RPtr);	//Ak existuje, uloží pravý podstrom na zásobník
        }
        tmp = *RootPtr;
        *RootPtr = (*RootPtr)->LPtr;	//Posunie sa do ľavého podstromu
        free(tmp);
