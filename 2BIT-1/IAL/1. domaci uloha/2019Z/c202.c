...
*/
		...
		//Inicializujeme vrchol zásobníka na -1 aby sme mohli začať od 0
		s->top = -1;
}
...
*/
		//Prázdny je v rovnakom stave ako inicializovaný
		return s->top == -1;
}
...
*/
		...
		if (stackEmpty(s))
			stackError(SERR_TOP);
		//Do *c sa uloží znak z vrcholu zásobníka
		else
			*c = s->arr[s->top];
}
...
*/
		...
		//Navýši vrchol o nový charakter c
		else {
			s->top++;
			s->arr[s->top] = c;
		}
